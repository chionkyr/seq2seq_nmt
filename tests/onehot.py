...
from thesis_lib.ds_utils.ds_map_functions import to_seq2seq_format, to_tokenize_input_target, to_one_hot_all
from thesis_lib.ds_utils.helpers import split_ds
from thesis_lib.metrics import sentence_gleu, sentence_bleu, sentence_wer, sentence_cer, corpus_gleu, corpus_bleu
...
from thesis_lib.preprocessing.utils import reverse_one_hot, get_string_lookup
...
train_ds_oh = train_ds.map(lambda x, y: to_tokenize_input_target(x, y, input_vocab_to_num, target_vocab_to_num, character_level))
train_ds_oh = train_ds_oh.map(lambda x, y: to_seq2seq_format(x, y, t_pad_value))
...
train_ds_oh = train_ds_oh.map(lambda x, y: to_one_hot_all(x, y, input_vocab_size, target_vocab_size))
train_ds_oh = train_ds_oh.prefetch(tf.data.experimental.AUTOTUNE)

val_ds_oh = val_ds.map(lambda x, y: to_tokenize_input_target(x, y, input_vocab_to_num, target_vocab_to_num, character_level))
val_ds_oh = val_ds_oh.map(lambda x, y: to_seq2seq_format(x, y, t_pad_value))
...
val_ds_oh = val_ds_oh.map(lambda x, y: to_one_hot_all(x, y, input_vocab_size, target_vocab_size))
val_ds_oh = val_ds_oh.prefetch(tf.data.experimental.AUTOTUNE)

test_ds_oh = test_ds.map(lambda x, y: to_tokenize_input_target(x, y, input_vocab_to_num, target_vocab_to_num, character_level))
test_ds_oh = test_ds_oh.map(lambda x, y: to_seq2seq_format(x, y, t_pad_value))
...
test_ds_oh = test_ds_oh.map(lambda x, y: to_one_hot_all(x, y, input_vocab_size, target_vocab_size))
test_ds_oh = test_ds_oh.prefetch(tf.data.experimental.AUTOTUNE)

print("batch_size: {0}".format(batch_size))
print("train_ds_oh length: {0} batches".format(len(train_ds_oh)))
print("val_ds_oh length: {0} batches".format(len(val_ds_oh)))
print("test_ds_oh length: {0} batches".format(len(test_ds_oh)))
...
# Testing
ds_len = len(test_ds_oh)
prog_bar = tf.keras.utils.Progbar(ds_len)

test_target = []
test_predicted = []
for i, batch in enumerate(test_ds_oh.as_numpy_iterator()):
	input_batch = batch[0][0]
	target_batch = batch[0][1]
	target = reverse_one_hot(target_batch, target_num_to_vocab, character_level)
	predicted = one_hot_model.decode_sequences(input_batch)
...
for batch in val_ds_oh.take(1).as_numpy_iterator():
	input_batch = batch[0][0]
	target_batch = batch[0][1]
	original = reverse_one_hot(input_batch, input_num_to_vocab, character_level)
	target = reverse_one_hot(target_batch, target_num_to_vocab, character_level)
	predicted = one_hot_model.decode_sequences(input_batch)
